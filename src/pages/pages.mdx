...
<CompactTable>
  <CompactRow>
    ...
    <CompactCell>Unique identifier for the page.</CompactCell>
  </CompactRow>
  <CompactRow>
...
## Create page content {{ tag: 'POST', label: '/v1/pages' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/v1/pages">

    ```bash {{ title: 'cURL' }}
    curl {{host}}/api/v1/pages \
      -H "Authorization: Bearer {admin_token}" \
      -d type="PRIVACY_POLICY" \
      -d content="Privacy policy content goes here."
    ```
...
## List all pages {{ tag: 'GET', label: '/v1/pages' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="GET" label="/v1/pages">

    ```bash {{ title: 'cURL' }}
    curl -G {{host}}/api/v1/pages \
      -H "Authorization: Bearer {admin_token}"
    ```
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="GET" label="/v1/pages/PRIVACY_POLICY">

    ```bash {{ title: 'cURL' }}
    curl {{host}}/api/v1/pages/PRIVACY_POLICY \
      -H "Authorization: Bearer {admin_token}"
    ```
    ...
    await client.pages.get('PRIVACY_POLICY')
    ```
    ...
    client.pages.get("PRIVACY_POLICY")
    ```
...
## Update page content {{ tag: 'PATCH', label: '/v1/pages/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="PATCH" label="/v1/pages/:id">

    ```bash {{ title: 'cURL' }}
    curl -X PATCH {{host}}/api/v1/pages/unique_page_id \
      -H "Authorization: Bearer {admin_token}" \
      -d content="Updated privacy policy content."
    ```
    ...
    await client.pages.update('unique_page_id', {
    ...
    client.pages.update("unique_page_id", {
      "content": "Updated privacy policy content.",
      "isActive": True,
    })
    ```
...
## Delete page content {{ tag: 'DELETE', label: '/v1/pages/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="DELETE" label="/v1/pages/:id">

    ```bash {{ title: 'cURL' }}
    curl -X DELETE {{host}}/api/v1/pages/unique_page_id \
      -H "Authorization: Bearer {admin_token}"
    ```
    ...
    await client.pages.delete('unique_page_id')
    ```
    ...
    client.pages.delete("unique_page_id")
    ```
