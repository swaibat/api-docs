...
<CompactTable>
  <CompactRow>
    ...
    <CompactCell>A unique identifier for the category. This defaults to `1` if not specified.</CompactCell>
  </CompactRow>
  <CompactRow>
  ...
  <CompactRow>
    ...
    <CompactCell type>number</CompactCell>
    <CompactCell>Indicates the parent category ID for hierarchical organization. Defaults to `0` if the category has no parent.</CompactCell>
  </CompactRow>
</CompactTable>

---

## Get All Categories {{ tag: 'GET', label: '/' }}
...
## Create Category {{ tag: 'POST', label: '/' }}
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X POST {{host}}/api/v1/categories \
      -H "Authorization: Bearer {user_token}" \
      --data '{
        "name": "drone",
        "slug": "drone",
...
## Update Category {{ tag: 'PUT', label: '/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="PUT" label="/:id">

    ```bash {{ title: 'cURL' }}
    curl -X PUT {{host}}/api/v1/categories/:id \
      -H "Authorization: Bearer {user_token}" \
      --data '{
...
## Delete Category {{ tag: 'DELETE', label: '/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="DELETE" label="/:id">

    ```bash {{ title: 'cURL' }}
    curl -X DELETE {{host}}/api/v1/categories/:id \
      -H "Authorization: Bearer {user_token}"
    ```
