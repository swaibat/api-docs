...
<CompactTable>
  <CompactRow>
    ...
    <CompactCell>Unique identifier for the address.</CompactCell>
  </CompactRow>
  <CompactRow>
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/v1/addresses">

    ```bash {{ title: 'cURL' }}
    curl {{host}}/api/v1/addresses \
...
## List all addresses {{ tag: 'GET', label: '/v1/addresses' }}
...
## Get addresses of the authenticated user {{ tag: 'GET', label: '/v1/addresses/me' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="GET" label="/v1/addresses/me">

    ```bash {{ title: 'cURL' }}
    curl -G {{host}}/api/v1/addresses/me \
      -H "Authorization: Bearer {user_token}"
    ```
...
## Update address {{ tag: 'PATCH', label: '/v1/addresses/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    ```bash {{ title: 'cURL' }}
    curl -X PATCH {{host}}/api/v1/addresses/unique_address_id \
      -H "Authorization: Bearer {user_token}" \
      --data '{
        "phoneNumber": "123456789",
        "email": "user@example.com",
...
## Delete address {{ tag: 'DELETE', label: '/v1/addresses/:id' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="DELETE" label="/v1/addresses/:id">

    ```bash {{ title: 'cURL' }}
    curl -X DELETE {{host}}/api/v1/addresses/unique_address_id \
      -H "Authorization: Bearer {user_token}" \
      --data '{
        "receiverId": "634dd5f9cd3498370afbdba4",
        "senderId": "63582d9ff8614efe18526c9e",
        "message": "hello doe"
      }'
    ```
